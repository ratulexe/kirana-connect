/**
 * App-wide loading screen: "Kirana" then "Connect" fade up on their own
 * lines, a progress bar underneath that fills once from empty to full.
 * Plain CSS animation, not GSAP -- see the keyframes in index.css for why.
 *
 ...
 *
 * `extraLine`, when given, renders as a third line below "Connect" (e.g.
 * "Store" / "Admin" / "Business") for apps that aren't the plain Consumer
 * wordmark.
 */
export default function AppLoader({ onDone, extraLine }) {
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center gap-7 bg-canvas" role="status">
      ...
      <span className="sr-only">Loading Kirana Connect</span>
    </div>
  );
}
